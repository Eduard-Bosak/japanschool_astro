  ...
  try {
    ...
    while((match = urlRegex.exec(cssText))){
      ...
      try {
        ...
        isDev && console.warn('[fonts] single font failed', fileName, err.message);
      }
    }
    ...
    isDev && console.warn('[fonts] skipped:', e.message);
    return { preload: null };
  }
}
...
async function buildBlog(site){
  ...
  for(const file of entries){
    ...
    if(!front.title){
      isDev && console.warn('[blog] warning: missing title in', file);
    }
    if(!front.date){
      isDev && console.warn('[blog] warning: missing date in', file);
    }
    let isoDate = date;
    try {
      if(date){
    ...
    workingBody = workingBody.replace(/\[\^([a-zA-Z0-9_-]+)\]/g, (match, fid) => {
      ...
      return `<sup class=\"fn-ref\" id=\"fnref-${fid}\"><a href=\"#fn-${fid}\" aria-label=\"������ ${n}\">${n}</a></sup>`;
    });
    ...
    if(footnotes.length){
      const list = footnotes.map(fn => {
        ...
        return `<li id=\"fn-${fn.id}\"><p>${fn.text} <a href=\"#fnref-${fn.id}\" class=\"fn-back\" aria-label=\"����� � ������\">?</a></p></li>`;
      }).filter(Boolean).join('');
      if(list){
        htmlBodyWithFootnotes += `<section class=\"footnotes\" aria-label=\"����������\"><ol>${list}</ol></section>`;
      }
    }
    const tocItems = [];
    htmlBodyWithFootnotes = htmlBodyWithFootnotes.replace(/<h([23])>([^<]+)<\/h\1>/g, (full, level, text) => {
      const base = text.replace(/<[^>]+>/g,'').trim();
      const hidRaw = base.toLowerCase().replace(/[^a-z0-9�-��\-\s]+/gi,'').replace(/\s+/g,'-').replace(/-+/g,'-').replace(/^-|-$/g,'');
      let hid = hidRaw;
      let dupe = 1;
    ...
    if(tocItems.filter(i => i.level===2 || i.level===3).length >= 3){
      ...
      tocHtml = `<nav class=\"post-toc\" aria-label=\"���������� ������\"><div class=\"toc-title\">����������</div><ol class=\"toc-list\">${tocList}</ol></nav>`;
    }
    let readingTime = 1;
    let wordCount = 0;
    try {
    ...
    const breadcrumbJson = JSON.stringify({
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 2, name: '����', item: site + '/blog/' },
 ...
 * RU: ����������� ����������� ������ � dist (manifest, favicon, offline ��������, service worker, �����������)
 */
async function copyStatic(extras){
  ...
  try {
    ...
    isDev && console.warn('[copy] images dir not found:', e.message);
  }
}
...
async function processImages(){
  ...
  try {
    ...
    isDev && console.warn('[images] sharp not available, skipping responsive generation');
    return null;
  }
  ...
  try {
    ...
    isDev && console.warn('[images] source directory not found:', imagesDir);
    return null;
  }
  ...
  for(const file of files){
    ...
    for(const w of sizes){
      try {
        ...
        isDev && console.warn('[images] failed size', file, w, e.message);
      }
    }
    try {
      ...
      isDev && console.warn('[images] placeholder/stat fail', file, e.message);
    }
  }
  await fs.writeFile(path.join(dist,'img-manifest.json'), JSON.stringify(manifest,null,2), 'utf8');
  isDev && console.log('[images] generated responsive variants');
  return manifest;
}

/**
 * EN: Convert string to URL-friendly slug
 * RU: ����������� ������ � URL-friendly slug
 */
function slugify(str){
  return str.toLowerCase().replace(/[^a-z0-9�-��\-\s]+/gi,'').replace(/\s+/g,'-').replace(/-+/g,'-').replace(/^-|-$/g,'');
}
...
async function build(){
  ...
  let siteConfig = { siteUrl: 'https://example.com', siteName: '����� ��������� �����' };
  try {
  ...
  await processImages().catch(e => isDev && console.warn('[images] pipeline error', e.message));
  const site = siteConfig.siteUrl.replace(/\/?$/,'');
  const postsMeta = await buildBlog(site).catch(e => {
    isDev && console.warn('[blog] build error', e.message); return [];
  });
  ...
  try {
    ...
    if(postsMeta.length && !/id="latest-posts"/.test(idxHtml)){
      ...
      const catsToolbar = allCats.length ? `<div class="posts-cats" role="toolbar" aria-label="������ �� ����������">${['<button type=\"button\" class=\"cat-filter active\" data-cat=\"__all\" aria-pressed=\"true\">���</button>', ...allCats.map(c => `<button type=\"button\" class=\"cat-filter\" data-cat=\"${c.replace(/"/g,'&quot;')}\" aria-pressed=\"false\">${c}</button>`)].join('')}</div>` : '';
      const cards = sorted.map((p,i) => {
        ...
        return `<article class=\"post-card fx-fade-up\" data-index=\"${i}\" data-delay=\"${Math.min((i+1)*70,420)}\" data-title=\"${p.title.replace(/"/g,'&quot;')}\" data-keywords=\"${(p.keywords||'').replace(/"/g,'&quot;')}\" data-desc=\"${(p.description||'').replace(/"/g,'&quot;')}\" data-cats=\"${(p.categories||[]).map(c => c.replace(/"/g,'&quot;')).join(',')}\">\n  ${badge}<a class=\"post-card-link\"${accentStyle} href=\"${site}/blog/${p.slug}/\">\n    <div class=\"post-card-media\" data-bg=\"${relCover}\" data-plc=\"${plc}\" data-state=\"loading\" style=\"background-image:url('${plc}');\"></div>\n    <div class=\"post-card-content\">\n      <div class=\"post-card-meta\"><time datetime=\"${p.date}\" class=\"post-card-date\">${formatDate(p.date)}</time><span class=\"sep\">�</span><span class=\"read-time\">${p.readingTime}&nbsp;���</span></div>\n      <h3 class=\"post-card-title\">${p.title}</h3>\n      <p class=\"post-card-desc\">${(p.description||'').slice(0,140)}</p>\n      ${cats?`<div class=\\"post-card-cats\\">${cats}</div>`:''}\n      <button type=\"button\" class=\"share-btn\" data-share=\"${site}/blog/${p.slug}/\" aria-label=\"����������� ������ �� ������\" title=\"����������� ������\">?</button>\n    </div>\n  </a>\n</article>`;
      }).join('\n');
      const snippet = `\n<section id="latest-posts" class="section latest-posts" aria-labelledby="latestPostsTitle"><div class="container"><div class="section-head fx-fade-up"><h2 id="latestPostsTitle">��������� ������</h2><p>����� ��������� �� �����: ��������, JLPT, ��������.</p></div><div class="posts-toolbar fx-fade-up" data-delay="40"><div class="search-bar"><input id="postSearch" type="search" placeholder="����� �� �������..." aria-label="����� �� �������" /><button type="button" id="postSearchReset" class="search-reset" aria-label="�������� �����" hidden>&times;</button><span id="postResultsCount" class="results-count" aria-live="polite"></span></div>${catsToolbar}</div><div class="post-cards">${cards}</div><div class="view-all-link fx-fade-up" data-delay="300"><a href="${site}/blog/" class="view-all-anchor">�������� ��� ������ ></a></div><div class="feed-callout fx-fade-up" data-delay="360"><div class="feed-callout-inner"><p class="feed-text">�����������: <a href="${site}/rss.xml">RSS</a> � <a href="${site}/atom.xml">Atom</a></p></div></div></div></section>\n`;
  ...
  try {
    ...
    isDev && console.log('[sitemap] sitemap.xml generated');
    const robots = `User-agent: *\nAllow: /\nSitemap: ${site}/sitemap.xml\n`;
    await fs.writeFile(path.join(dist,'robots.txt'), robots, 'utf8');
    isDev && console.log('[robots] robots.txt generated');
  } catch(e){
    isDev && console.warn('[sitemap] generation error', e.message);
  }
  try {
    if(postsMeta.length){
      ...
      const rss = `<?xml version="1.0" encoding="UTF-8"?><rss version="2.0"><channel><title>���� � ${siteConfig.siteName}</title><link>${site}/blog/</link><description>������ ${siteConfig.siteName}</description><lastBuildDate>${new Date(updated).toUTCString()}</lastBuildDate>${rssItems}</channel></rss>`;
      ...
      const atom = `<?xml version="1.0" encoding="UTF-8"?><feed xmlns="http://www.w3.org/2005/Atom"><id>${site}/blog/</id><title>���� � ${siteConfig.siteName}</title><updated>${updated}</updated><link href="${site}/atom.xml" rel="self"/><link href="${site}/blog/"/>${atomEntries}</feed>`;
      await fs.writeFile(path.join(dist,'atom.xml'), atom, 'utf8');
      isDev && console.log('[feed] rss.xml & atom.xml generated');
    }
  } catch(e){
    isDev && console.warn('[feed] generation error', e.message);
  }
  ...
  try {
    ...
    isDev && console.warn('[canonical] injection error', e.message);
  }
  try {
    await generateOgImage();
  } catch(e){
    isDev && console.warn('[og] generation skipped:', e.message);
  }
  isDev && console.log('[build] done:', cssName, jsName);
}
...
async function main(){
  if(isClean){
    await rimraf(dist); isDev && console.log('dist cleaned'); return;
  }
  await rimraf(dist);
  await build();
  if(isWatch){
    isDev && console.log('[watch] watching for changes...');
