...
async function buildFonts(){
  try {
    ...
    while((match = urlRegex.exec(cssText))){
      ...
      if(found.includes(fileName)) continue;
      try {
    ...
    let preloadCandidate = found.find(f=>/playfair/i.test(f)) || found[0] || null;
...
async function buildBlog(site){
  ...
  let entries = [];
  try { entries = await fs.readdir(contentDir); } catch{ return []; }
  ...
  if(!front.title){ console.warn('[blog] warning: missing title in', file); }
  if(!front.date){ console.warn('[blog] warning: missing date in', file); }
  let isoDate = date;
  try { if(date){ const d = new Date(date); if(!isNaN(d.getTime())) isoDate = d.toISOString(); } } catch{}
    let dateModified = '';
    try { const st = await fs.stat(abs); dateModified = st.mtime.toISOString(); } catch {}
    ...
    workingBody = workingBody.replace(/\[\^([a-zA-Z0-9_-]+)\]/g, (match, fid)=>{
      if(!refOrderMap[fid]) refOrderMap[fid] = refIndex++;
    ...
    htmlBodyWithFootnotes = htmlBodyWithFootnotes.replace(/<h([23])>([^<]+)<\/h\1>/g, (full, level, text)=>{
      ...
      let hid = hidRaw;
      let dupe = 1;
      while(tocItems.find(i=>i.id===hid)) hid = hidRaw + '-' + (++dupe);
    ...
    try {
      ...
      if(Array.isArray(rawCats)) categories = rawCats.filter(Boolean).map(c=>String(c).trim()).filter(Boolean);
      ...
      try {
        if(imageManifest){
          ...
          if(!entry){
            ...
            if(key) entry = imageManifest[key];
          }
          if(entry){
  ...
  try {
    if(coverFileResolved && imageManifest){
      ...
      if(!entry2){
        ...
        if(key2) entry2 = imageManifest[key2];
      }
      if(entry2 && entry2.placeholder){ placeholder = entry2.placeholder; }
    }
  } catch{}
  let accent = null;
  try {
  ...
  postsMeta.push({ title, slug, description: desc, date: isoDate, modified: dateModified || isoDate, cover: coverFileResolved, keywords, readingTime, wordCount, categories, placeholder, accent });
    ...
    const localeDate = (()=>{ try { return new Date(isoDate).toLocaleDateString('ru-RU',{ day:'2-digit', month:'short', year:'numeric'}).replace('.', ''); } catch { return isoDate.split('T')[0]; } })();
  ...
  if(indexItems.length){
    ...
    const formatted = indexItems.sort((a,b)=> (a.date < b.date ? 1 : -1)).map(it=>{
      ...
      const dateShort = (()=>{ try { return new Date(it.date).toLocaleDateString('ru-RU',{ day:'2-digit', month:'short'}).replace('.', ''); } catch { return ''; } })();
...
async function copyStatic(extras){
  ...
  for(const f of staticFiles){
    try { await fs.copyFile(path.join(root,'public',f), path.join(dist,f)); } catch {}
  }
  ...
  } catch(e){ console.warn('[copy] images dir not found:', e.message); }
}
...
async function processImages(){
  ...
  for(const file of files){
    if(!targetExt.test(file)) continue;
    ...
    } catch(e){ console.warn('[images] placeholder/stat fail', file, e.message); }
  }
...
async function findCurrent(type){
  ...
  if(type === 'css') return files.find(f=>/^styles\.[a-f0-9]+\.css$/.test(f));
  if(type === 'js') return files.find(f=>/^main\.[a-f0-9]+\.js$/.test(f));
  return '';
}
...
function patchIndex(html, cssName, jsName, fontPreload){
  ...
  const inject = [fontPreload ? `<link rel="preload" href="fonts/${fontPreload}" as="font" type="font/woff2" crossorigin>` : '', '<link rel="stylesheet" href="fonts.css" />'].filter(Boolean).join('\n    ');
  ...
  html = html.replace(`<link rel="stylesheet" href="${cssName}"/>`, `<link rel="stylesheet" href="${cssName}" media="print" onload="this.media='all'">`);
  return html;
}
...
async function build(){
  ...
  if(argSite){ siteConfig.siteUrl = argSite.split('=')[1]; }
  ...
  const postsMeta = await buildBlog(site).catch(e=> { console.warn('[blog] build error', e.message); return []; });
  ...
  try {
    ...
    if(postsMeta.length && !/id="latest-posts"/.test(idxHtml)){
      ...
      const now = Date.now();
      const isNew = d=> { try { return (now - new Date(d).getTime()) / 86400000 < 14; } catch { return false; } };
      const formatDate = d=>{ try { return new Date(d).toLocaleDateString('ru-RU',{ day:'2-digit', month:'short'}).replace('.', '') } catch { return '' } };
  ...
  try {
    ...
    function metaFor(loc){
      if(loc === `${site}/`) return { changefreq:'weekly', priority:'1.0' };
      if(loc === `${site}/blog/`) return { changefreq:'daily', priority:'0.9' };
      return { changefreq:'monthly', priority:'0.7' };
    }
    const sitemap = `<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<urlset xmlns=\"http://www.sitemaps.org/schemas/sitemap/0.9\">\n${urls.map(u=>{ const m=metaFor(u.loc); return `<url><loc>${u.loc}</loc>${u.lastmod?`<lastmod>${u.lastmod}</lastmod>`:''}<changefreq>${m.changefreq}</changefreq><priority>${m.priority}</priority></url>`; }).join('\n')}\n</urlset>`;
    await fs.writeFile(path.join(dist,'sitemap.xml'), sitemap, 'utf8');
    console.log('[sitemap] sitemap.xml generated');
    const robots = `User-agent: *\nAllow: /\nSitemap: ${site}/sitemap.xml\n`;
    await fs.writeFile(path.join(dist,'robots.txt'), robots, 'utf8');
    console.log('[robots] robots.txt generated');
  } catch(e){ console.warn('[sitemap] generation error', e.message); }
  try {
    if(postsMeta.length){
  ...
  } catch(e){ console.warn('[feed] generation error', e.message); }
  ...
  } catch(e){ console.warn('[canonical] injection error', e.message); }
  try { await generateOgImage(); } catch(e){ console.warn('[og] generation skipped:', e.message); }
 ...
 */
async function main(){
  if(isClean){ await rimraf(dist); console.log('dist cleaned'); return; }
  await rimraf(dist);
  await build();
  if(isWatch){
    ...
    import('chokidar').then(({ default: chokidar })=>{
      chokidar.watch(['public/index.html','src/styles.css','src/scripts/main.js','public/service-worker.js','public/offline.html','public/manifest.json'], { ignoreInitial:true, cwd: root })
        .on('all', async ()=>{ try { await build(); } catch(e){ console.error(e); } });
    });
  }
}
